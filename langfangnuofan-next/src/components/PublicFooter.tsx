...
export default function PublicFooter() {
  const [info, setInfo] = useState({ phone: '', email: '', address: '', facebook_url: '', whatsapp_url: '' });
  const { t } = useLang();

  useEffect(() => {
    fetch('/api/company').then(r => r.json()).then(d => {
      if (d && typeof d === 'object') setInfo(d as { phone: string; email: string; address: string; facebook_url: string; whatsapp_url: string });
    }).catch(() => {});
  }, []);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              {(info.facebook_url || info.whatsapp_url) && (
                <li className="flex gap-2 pt-1">
                  {info.facebook_url && (
                    <a href={info.facebook_url} target="_blank" rel="noopener noreferrer"
                       className="inline-flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-1.5 rounded-full transition-colors">
                  ...
                  {info.whatsapp_url && (
                    <a href={info.whatsapp_url} target="_blank" rel="noopener noreferrer"
                       className="inline-flex items-center gap-1.5 bg-green-500 hover:bg-green-600 text-white text-xs px-3 py-1.5 rounded-full transition-colors">
